...
Usage:
    cd src && python test_services.py
"""
...
from src.services.service_factory import ServiceFactory, ServiceStatus
from src.services.health_monitor import HealthMonitor, AlertLevel
...
def test_health_monitor(factory):
    ...
    try:
        ...
        print(f"\n   HEALTH SUMMARY:")
        print(f"     Total checks: {health_summary['monitor_status']['total_checks']}")
        print(f"     Failed checks: {health_summary['monitor_status']['failed_checks']}")
        print(f"     Alerts generated: {len(alerts_received)}")
        print(f"     Monitoring time: {health_summary['monitor_status']['uptime']:.1f}s")
...
def main():
    ...
    try:
        ...
        print(f"   Embedding Model: {config.ai_model.embedding_model_name}")
    except Exception as e:
